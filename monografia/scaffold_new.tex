...
para a leitura deste texto.
    ...
    %% A segmentação de páginas de documentos é uma etapa importante da análise de documentos pois nela é possível isolar um ou mais tipos específicos de conteúdo, tais como: blocos de texto, gráficos, imagens, títulos, legendas, separadores, tabelas, fórmulas matemáticas e regiões com ruído.

    %% Diversas soluções computacionais foram propostas para o problema desde o surgimento do campo de pesquisa em visão computacional. Automatizar esta tarefa reduz custos, aumenta a velocidade e capacidade de processamento de documentos além de possivelmente reduzir a taxa de erro humano na classificação de uma região.

    %% Quantificar a qualidade destas soluções e compará-las entre si tem sido o foco do ICDAR (International Conference on Document Analisys and Recognition) o qual estabelece métricas precisas e algoritmos para obtê-las. Utilizaremos os padrões adotados nestas competições para evitar qualquer forma de subjetividade na avaliação da qualidade da solução.
